    ...
    foreach ($proc in $running) {
        try {
            ...
            if ($SAFE_TO_KILL -contains $name) {
                ...
                if ($memMB -lt $thresholdMB) { continue }
...
function Invoke-Cleaner {
    ...
    foreach ($proc in $Processes) {
        ...
        try {
            # Add -Force to ensure stubborn bloat processes actually die
            ...
            if ($msg -match "Cannot find a process" -or $msg -match "process with process id") {
                $alreadyGone += 1
                Write-Host "  " -NoNewline
                # FIX: Single quotes prevent the '--' unary operator error
